...
public:
  ...
  LGFX(void)
  {
    { // バス制御の設定を行います。
      ...
      // SPIバスの設定
      // cfg.spi_host = VSPI_HOST;     // 使用するSPIを選択  ESP32-S2,C3 :
      // SPI2_HOST or SPI3_HOST / ESP32 : VSPI_HOST or HSPI_HOST ※
      ...
      // cfg.use_lock   = true;        //
      // トランザクションロックを使用する場合はtrueを設定 cfg.dma_channel =
    ...
    { // 表示パネル制御の設定を行います。
      ...
      //   (4~7は上下反転) cfg.dummy_read_pixel = 8; //
      //   ピクセル読出し前のダミーリードのビット数 cfg.dummy_read_bits = 1; //
      //   ピクセル以外のデータ読出し前のダミーリードのビット数
